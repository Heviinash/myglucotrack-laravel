...
@section('page-title', 'Add Blood Sugar Record')
        ...
        <form method="POST" action="{{ route('blood-sugar.store') }}">
            ...
            <div>
                ...
                <select name="before_after" class="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-blue-500 focus:border-blue-500" required>
                    <option value="Before Meal" {{ old('before_after') == 'Before Meal' ? 'selected' : '' }}>Before Meal</option>
                    <option value="After Meal" {{ old('before_after') == 'After Meal' ? 'selected' : '' }}>After Meal</option>
                </select>
            </div>
            ...
            <div class="grid grid-cols-2 gap-4">
                ...
                <div>
                    ...
                    <input type="time" name="measurement_time"
                           value="{{ old('measurement_time', date('H:i')) }}"
                           class="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-blue-500 focus:border-blue-500" required>
                </div>
            </div>
            ...
            <div>
                ...
                <textarea name="notes" rows="3" placeholder="Any relevant observations..."
                          class="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-blue-500 focus:border-blue-500">{{ old('notes') }}</textarea>
            </div>
