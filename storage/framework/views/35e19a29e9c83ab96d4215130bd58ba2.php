...
<div class="max-w-7xl mx-auto px-4 sm:px-6">
    ...
    <div class="bg-white rounded-xl border border-gray-100 shadow-sm p-3 sm:p-4 mb-6">
        <form method="GET" action="<?php echo e(route('patients.index')); ?>" class="flex flex-col sm:flex-row flex-wrap gap-2 sm:gap-3 items-start sm:items-end">
            
            <div class="w-full sm:flex-1">
                <label class="block text-xs text-gray-500 mb-1">Search</label>
                <input type="text" name="search" value="<?php echo e(request('search')); ?>" placeholder="Name, IC number, or phone..."
            ...
            <div class="w-full sm:w-auto">
                ...
                <input type="number" name="age_from" value="<?php echo e(request('age_from')); ?>" placeholder="Min"
                       class="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm">
            </div>

            <div class="w-full sm:w-auto">
                <label class="block text-xs text-gray-500 mb-1">Age To</label>
                <input type="number" name="age_to" value="<?php echo e(request('age_to')); ?>" placeholder="Max"
                       class="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm">
            </div>
            ...
            <div class="flex gap-2 w-full sm:w-auto">
                ...
                <?php if(request()->hasAny(['search', 'gender', 'age_from', 'age_to'])): ?>
            ...
            <table class="w-full text-sm">
                ...
                <tbody class="divide-y divide-gray-100">
                    ...
                    <tr class="hover:bg-gray-50 transition">
                        <td class="px-4 sm:px-5 py-3 text-gray-400 text-xs sm:text-sm"><?php echo e($loop->iteration); ?></td>
                        <td class="px-4 sm:px-5 py-3 font-medium text-gray-800">
                            <a href="<?php echo e(route('patients.show', $patient->id)); ?>" class="hover:text-blue-600 transition text-sm sm:text-base">
                        ...
                        <td class="hidden sm:table-cell px-4 sm:px-5 py-3 text-gray-600 font-mono tracking-wide text-xs">
                            <?php echo e($patient->ic_number ? substr($patient->ic_number,0,6).'-'.substr($patient->ic_number,6,2).'-'.substr($patient->ic_number,8,4) : '—'); ?>

                        </td>
                        <td class="hidden md:table-cell px-4 sm:px-5 py-3 text-gray-600 text-xs sm:text-sm">
                            <?php echo e($patient->dob ? $patient->dob->format('d M Y') : '—'); ?>

                        </td>
                        <td class="px-4 sm:px-5 py-3 text-gray-600 text-xs sm:text-sm"><?php echo e($patient->age); ?> yrs</td>
                        <td class="hidden sm:table-cell px-4 sm:px-5 py-3">
                            <span class="px-2 py-1 rounded-full text-xs font-medium
                                <?php echo e($patient->gender === 'Male' ? 'bg-blue-100 text-blue-700' : 'bg-pink-100 text-pink-700'); ?>">
                                <?php echo e($patient->gender ?? '—'); ?>

                            </span>
                        </td>
                        <td class="hidden lg:table-cell px-4 sm:px-5 py-3 text-gray-600 text-xs"><?php echo e($patient->phone ?? '—'); ?></td>
                            ...
                            <div class="flex items-center gap-2 flex-wrap">
                                ...
                                <?php if(auth()->user()->role !== 'User'): ?>
                                    ...
                                    <form action="<?php echo e(route('patients.destroy', $patient->id)); ?>" method="POST" class="inline"
                                          onsubmit="return confirm('Delete <?php echo e($patient->patient_name); ?>?')">
                    ...
                    <tr>
                        <td colspan="8" class="px-4 sm:px-5 py-10 text-center text-gray-400 text-sm">
                            No patients found.
                            <?php if(auth()->user()->role !== 'User'): ?>
                                <a href="<?php echo e(route('patients.create')); ?>" class="text-blue-600 hover:underline ml-1">Add first patient →</a>
                            <?php endif; ?>
                        </td>
                    </tr>
                    <?php endif; ?>
...
<?php echo $__env->make('layouts.app', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH C:\xampp\htdocs\myglucotrack-laravel-v3\resources\views/patients/index.blade.php ENDPATH**/ ?>